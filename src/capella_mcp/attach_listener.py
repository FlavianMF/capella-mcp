...
Request bodies are the exact same text bridge.py's spawn-mode headless
calls already use -- they still read `model = CapellaModel();
model.open(<workspace-relative path>)`, unmodified. But that path string
only means anything inside bridge.py's own throwaway spawn-mode project
layout, not this listener's real, persistent GUI workspace. So the
branching happens here instead, invisibly to the body: each request
carries the target model's absolute path alongside its script text,
_process_one_request() resolves the matching already-open Session for
it (_sessions_by_path()) and injects a CapellaModel subclass whose
open() just binds to that session directly instead of trying to
re-resolve the (here, meaningless) path string -- see
_bound_capella_model()'s docstring. (Confirmed live, separately: two
CapellaModel().open() calls on the SAME literal path string, within one
JVM, do reuse rather than duplicate the session --
SessionManager.INSTANCE.getSessions() stayed at size 1 across both. That
result motivated trusting "the session is already there, just bind to
it" as the right model here, even though the actual binding below
doesn't call the real open() at all.)
"""
...
def _sessions_by_path():
    """{abs filesystem path: live Sirius Session java object} for every
    session currently open in this workspace. Used both for the
    heartbeat (just the keys) and, critically, to bind a request's
    CapellaModel to the EXISTING session for its target path instead of
    calling .open() with a path string that only makes sense inside
    bridge.py's own throwaway spawn-mode project layout -- see
    _bound_capella_model()'s docstring."""
...
def _open_model_paths():
    return list(_sessions_by_path().keys())


def _bound_capella_model(real_capella_model_cls, session):
    """A CapellaModel subclass whose open() ignores the path argument it's
    given and binds straight to an already-resolved live Session instead.

    Why this exists: every request body bridge.py sends is the exact same
    text it would use for a fresh spawn-mode headless call, which does
    `model = CapellaModel(); model.open(<workspace-relative path>)` --
    that path string (e.g. "/models/demo.aird") only resolves inside the
    throwaway Eclipse project bridge.py's own spawn mode auto-imports for
    every headless call (_ensure_models_project()). This listener's real,
    persistent GUI workspace has no reason to contain a project with that
    same name/layout, so calling the real CapellaModel.open() with that
    string here would fail (or, worse, silently resolve to an unrelated
    resource). Since attach mode's whole premise is "the session for this
    path is ALREADY open" (that's what _attach_target() matched on in
    bridge.py before ever sending the request), the correct thing is to
    just bind to it directly and skip the open() call's real work
    entirely -- every other CapellaModel method (get_system_engineering,
    start_transaction, save, ...) only ever touches self.session, so
    nothing else needs to change."""

    class _BoundCapellaModel(real_capella_model_cls):
        def open(self, _ignored_workspace_path):
            self.session = session

    return _BoundCapellaModel
...
def _process_one_request(req_path, result_path):
    with open(req_path, "r") as f:
        payload = json.load(f)
    body = payload["body"]
    target_path = payload.get("model_path")
    ...
    session = _sessions_by_path().get(target_path) if target_path else None
    if target_path and session is None:
        # Rare race: the heartbeat that got this request routed here was
        # already stale by the time it arrived (model closed in the GUI
        # in between). Not _AttachUnavailable -- the listener IS reachable
        # -- so bridge.py surfaces this as a normal BridgeError rather
        # than silently retrying via spawn; the NEXT call's heartbeat scan
        # will simply stop finding this attach target.
        _write_result({"error": f"model no longer open in this GUI session: {target_path}"})
        return

    local_ns = dict(globals())
    local_ns["_write_result"] = _write_result
    if session is not None:
        local_ns["CapellaModel"] = _bound_capella_model(local_ns["CapellaModel"], session)
    try:
