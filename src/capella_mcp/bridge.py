...
def _run_script_attach(
    script_body: str, attach: AttachInfo, model_path: Path, timeout: float = ATTACH_REQUEST_TIMEOUT_SECONDS
) -> dict:
    ...
    model_path (absolute) rides along separately from script_body: the
    body still contains a spawn-mode-style workspace-relative
    `model.open(...)` call (unchanged, reused verbatim from the headless
    template), but that string is meaningless inside the listener's real
    GUI workspace. attach_listener.py uses model_path instead to bind
    CapellaModel to the already-open Session directly -- see its
    _bound_capella_model()."""
    ...
    request_path.write_text(
        json.dumps({"body": script_body, "model_path": str(model_path)}), encoding="utf-8"
    )
...
def _dispatch(abs_path: Path, body: str, timeout: float = DEFAULT_TIMEOUT) -> dict:
    ...
    if attach is not None:
        try:
            return _run_script_attach(body, attach, abs_path)
...
def list_layers(model_path: str) -> dict:
    """Dispatcher: attach (live GUI, see docs/decisions/0006) first if one
    has this model open -- it's the freshest source (sees unsaved GUI
    edits) and _list_layers_headless's own _dispatch() call would find it
    anyway, so checking here just skips a wasted fast_reader/disk read.
    Otherwise capellambse fast-path (ms, no Capella process, but only
    reflects the last *saved* state), then the headless template as final
    fallback. See docs/decisions/0005-camada-leitura-capellambse.md."""
    abs_path = resolve_model_path(model_path)
    if _attach_target(abs_path) is not None:
...
def list_elements(model_path: str, layer: str, type_filter: str | None = None) -> dict:
    """Dispatcher -- see list_layers docstring for the attach-first
    rationale. fast_reader only handles the type_filter-given case (see
    its own comment for why the no-filter shape has no clean capellambse
    equivalent); a missing type_filter falls straight through to
    headless."""
